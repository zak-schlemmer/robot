...
case $2 in
    ...
    restart )
        docker-sync stop -c /etc/robot/docker-sync/${project}-docker-sync.yml --dir ~/robot.dev/docker-sync"${where}"
        sleep 2
        docker-sync start -c /etc/robot/docker-sync/${project}-docker-sync.yml --dir ~/robot.dev/docker-sync"${where}" --daemon
        ;;
